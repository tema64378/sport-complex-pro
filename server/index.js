...
// --- Workout Templates ---
app.get('/api/workout-templates', authRequired, async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM workout_templates ORDER BY category ASC, name ASC');
    res.json(rows);
  } catch (e) {
...
app.get('/api/workout-templates/:id', authRequired, async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM workout_templates WHERE id = ?', [req.params.id]);
    if (!rows[0]) return res.status(404).json({ error: 'Template not found' });
    res.json(rows[0]);
  } catch (e) {
    res.status(500).json({ error: 'Fetch template failed' });
  }
});
